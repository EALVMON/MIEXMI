...
data class ComisionModelo(
    val idComision: Int,   // Guarda el ID (número entero)
    val nombre: String,    // Guarda el nombre de la comisión (texto)
...
class ComisionAdaptador(
    // Recibe la lista completa con todas las comisiones que hay que mostrar.
    private val listaDatos: List<ComisionModelo>,

    // Usamos una función Lambda para saber cuándo el usuario toca una fila.
    private val listener: (ComisionModelo) -> Unit
) : RecyclerView.Adapter<ComisionAdaptador.ComisionViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ComisionViewHolder {
        // LayoutInflater coge el archivo de diseño XML (item_comision) y lo "infla",
        // transformando ese código visual en un objeto real que la pantalla puede pintar.
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_comision, parent, false)
        // devuelvo esa vista ya fabricada
        return ComisionViewHolder(view)
    }

    // RELLENAR LOS DATOS
    override fun onBindViewHolder(holder: ComisionViewHolder, position: Int) {
        // Busca en nuestra lista de datos la linea de nuestra comisión que toca dibujar
        ...
        // Rellenamos los textos de la fila con los datos reales que tiene nuestra comisión
        ...
        // Si el usuario toca esa fila, activamos el 'listener' y enviamos los datos de la comisión que tocó.
    ...
    class ComisionViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {

        // Enlazamos las variables de Kotlin con los IDs que creé en el diseño XML.
        val tvNumFila: TextView = itemView.findViewById(R.id.tvItemNumComision)
        val tvNombre: TextView = itemView.findViewById(R.id.tvItemNombreComision)
        val tvFechaBod: TextView = itemView.findViewById(R.id.tvItemFechaComision)
        val tvNumBod: TextView = itemView.findViewById(R.id.tvItemBodComision)
    }
}