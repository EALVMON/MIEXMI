...
public class Comisiones extends Temporizador {
    ...
    private int idComisionSeleccionada = -1; // La inicializo a -1. Si hubiera algo seleccionado,
    ...
    private List<ComisionModelo> listaDatos;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Conectamos el Adaptador que está en Kotlin (ComisionAdaptador.kt)
        adaptador = new ComisionAdaptador(listaDatos, comision -> {
            // CUANDO EL USUARIO TOCA UNA FILA DE LA LISTA:
            // 1. Guardamos el ID de esa comisión para saber cuál modificar/borrar
            idComisionSeleccionada = comision.getIdComision();
            // 2. Rellenamos las cajas de texto de arriba con los datos de la fila que tocó
            etNombreComision.setText(comision.getNombre());
            etFechaBod.setText(comision.getFechaBod());
            // Si el boletín es 0 (no tiene), lo dejamos en blanco
            etNumBod.setText(comision.getNumBod().equals("0") ? "" : comision.getNumBod());
        ...
        btnAnadir.setOnClickListener(v -> {
            ...
            // Lo guardamos en la Base de Datos, mediante el método anadirComision que está en ExpedienteHelper
    ...
    private void limpiarFormulario() {
        ...
        etFechaBod.setText("");
        etNumBod.setText("");
        idComisionSeleccionada = -1; // Vuelve al estado inicial
    }
    ...
    private void cargarLista() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                // Instanciamos la data class que teníamos creada en ComisionAdaptador en Kotlin
                // y la metemos en nuestra lista de Java
                listaDatos.add(new ComisionModelo(id, nombre, fecha, String.valueOf(nBod)));
